...
if predict_btn:
    ...
    # B. Prepare Payload (MATCHING TRAINING SCHEMA)
    # Note: 'Delivery_Date' is excluded because the training model dropped it.
    input_data = {
        "columns": [
            "Carrier", 
            "Origin_Warehouse", 
        ...
        "data": [[
            ...
            origin_name, 
            dest_name, 
            "December",      # Static for now (or derive from date)
            real_distance, 
            weight, 
            cost, 
            "On Time"        # Assuming 'Status' is required by model
        ]]
    }
